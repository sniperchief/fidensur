...
//
// FIDENSUR_CONTRACT is written alongside INSTRUCTION_SENDER because docker-compose passes it to the
// engine, which refuses to start without knowing which deployment it serves.
func writeEnv(path string, extensionID *big.Int, sender common.Address) error {
	content := fmt.Sprintf(
		"# Generated by register-extension. Do not edit by hand.\n"+
			...
			"FIDENSUR_CONTRACT=%s\n",
		extensionID, sender.Hex(), sender.Hex(),
	)
