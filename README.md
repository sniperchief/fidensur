...
| `frontend/lib/*.ts` | Complete | ⚠️ **Not yet type-checked** — dependency install was still running |
| `frontend/lib/__tests__/verify.test.ts` | Written | ⚠️ **Not yet run** |
| `frontend/app/verify/[round]/page.tsx` | Complete | ⚠️ Not type-checked |
| Organization console / recipient portal UI | Not written | — |
| `docker-compose.yaml`, `extension/Dockerfile` | Written | ⚠️ **Never built** — Docker is not installed on the development machine |

### What is and is not proven

**Proven by a passing test run** (127 tests: 101 Forge + 26 Go):
...
- The Docker image has never been built, so **no code hash exists** and reproducibility is a claim
  about the Dockerfile rather than an observed fact.
- The frontend has not been type-checked and its tests have not run.
...
| TEE signature chain | `test/GenerateSigVectors.t.sol` | `frontend/lib/__tests__/verify.test.ts` | Written, not yet run |
