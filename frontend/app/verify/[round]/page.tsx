...
export default function VerifyRoundPage({ params }: { params: { round: string } }) {
  ...
  if (!round) return <Shell><Callout kind="fail">Round not found.</Callout></Shell>;
    ...
    <Shell>
      <header>
        ...
        <dl className="summary">
          ...
          <div><dt>Organization</dt><dd><code>{round.organization}</code></dd></div>
          <div><dt>Recipients</dt><dd>{round.recipientCount || "—"}</dd></div>
          <div><dt>Allocated</dt><dd>{formatAmount(round.totalAllocated)} {symbol}</dd></div>
        </dl>
      </header>
      ...
      >
        {!PROXY_URL && <Callout kind="unknown">No proxy URL configured, so attestation cannot be checked from this page.</Callout>}
        {PROXY_URL && !attestation && <Callout kind="unknown">The proxy did not return an attestation report. It may be offline.</Callout>}
        {attestation && verdict && (
          <>
            ...
            <Check ok={!verdict.isSimulatedCodeHash} label="Code hash is a real measurement">
              <code>{abbreviate(attestation.codeHash)}</code>
              {verdict.isSimulatedCodeHash && (
                <em> — this is the simulated hash. Results from this machine are not FTDC-accepted.</em>
              )}
            </Check>
            <Check ok={verdict.matchesRegisteredExtension} label="Extension ID matches on-chain registration">
              <code>{attestation.extensionId}</code>
            </Check>
            {!verdict.trustworthy && (
              <Callout kind="warn">
                This deployment is running with simulated or unverified attestation. Everything below
                still demonstrates the mechanism, but it does not establish that a genuine enclave
                produced the result.
              </Callout>
            )}
          </>
      ...
      >
        {attestation ? (
          <>
            <Field label="Attested code hash"><code>{attestation.codeHash}</code></Field>
            <p>
              The Go image is bit-for-bit reproducible across machines. Rebuild the published commit
...
docker build --build-arg SOURCE_DATE_EPOCH=$(git log -1 --format=%ct) \\
  -t fidensur-extension ./extension`}</pre>
      ...
      >
        {!result && <Callout kind="unknown">No signed result is available from the proxy for this round's instruction.</Callout>}
        {result && result.status !== 1 && (
          <Callout kind="fail">The TEE reported status {result.status}. Only status 1 is relayable.</Callout>
        )}
        {steps && (
          <>
            <Field label="keccak256(resultData)"><code>{steps.dataHash}</code></Field>
            <Field label="ActionResult.Hash()"><code>{steps.resultHash}</code></Field>
            <Field label="Domain-separated payload"><code>{steps.payloadHash}</code></Field>
            <Field label="EIP-191 digest"><code>{steps.eip191Digest}</code></Field>
            <Check ok={steps.signerOk} label="Recovered signer is the registered TEE">
              <code>{steps.recoveredSigner ?? "recovery failed"}</code>
              {!steps.signerOk && <em> — expected <code>{steps.expectedSigner}</code></em>}
            </Check>
            <p className="note">
              The payload hash wraps the result hash in{" "}
              <code>abi.encode(bytes32(&quot;TEE_ACTION_RESULT&quot;), chainId, resultHash)</code>.
              Binding <code>chainId</code> ({steps.chainId}) is what prevents this signature being
      ...
      >
        <Check ok={round.totalAllocated <= round.funded} label="Allocated does not exceed funded">
          {formatAmount(round.totalAllocated)} ≤ {formatAmount(round.funded)} {symbol}
        </Check>
        <Check ok={round.totalClaimed <= round.totalAllocated} label="Claimed does not exceed allocated">
          {formatAmount(round.totalClaimed)} ≤ {formatAmount(round.totalAllocated)} {symbol}
        </Check>
        <Field label="Unclaimed">
          {formatAmount(round.totalAllocated - round.totalClaimed)} {symbol}
      ...
      >
        <Field label="Policy commitment"><code>{round.policyCommitment}</code></Field>
        <Field label="Compute instruction"><code>{round.computeInstructionId}</code></Field>
        <p className="note">
          <code>requestCompute</code> rejects any ciphertext that does not hash to the commitment, and{" "}
          <code>finalizeRound</code> rejects any result reporting a different one. An organization can
          publish its ciphertext later; recomputing <code>keccak256</code> over it and comparing to the
          value above proves which bytes the enclave evaluated — without revealing the policy.
        </p>
      </Section>
      ...
      >
        <Field label="Merkle root"><code>{round.merkleRoot}</code></Field>
        <Field label="Recipients">{round.recipientCount}</Field>
        <Field label="Engine version"><code>{decodeVersion(round.engineVersion)}</code></Field>
        <p className="note">
          Individual addresses and amounts are not on-chain and are not derivable from the root. A
          recipient proves their own entry with a Merkle proof; nobody learns anyone else&apos;s.
        </p>
        <Callout kind="warn">
          Claiming is self-disclosure. A settled transfer of <em>N</em> to address <em>A</em> is
          publicly visible, like any other transfer. Fidensur keeps unclaimed allocations private and
          never reveals the distribution as a whole, but it cannot make a settled payment invisible.
        </Callout>
      </Section>
...
function Shell({ children }: { children: React.ReactNode }) {
  return <main className="verify">{children}</main>;
}
...
function Callout({ kind, children }: { kind: "pass" | "fail" | "warn" | "unknown"; children: React.ReactNode }) {
